...
export class DeleteFormSubmissionExamples {
  ...
  static async testDeleteFormSubmissionExamples() {
    ...
    try {
      const validExample = this.getValidDeleteExample();
    ...
    console.log('\n❌ Teste 2: Formulário não encontrado');
    try {
      const nonExistentForm = this.getNonExistentFormExample();
    ...
    console.log('\n❌ Teste 3: Submissão não encontrada');
    try {
      const nonExistentSubmit = this.getNonExistentSubmitExample();
    ...
    console.log('\n❌ Teste 4: Submissão já removida');
    try {
      const alreadyRemoved = this.getAlreadyRemovedSubmitExample();
    ...
    console.log('\n❌ Teste 5: Formulário inativo');
    try {
      const inactiveForm = this.getInactiveFormExample();
    ...
    console.log('\n❌ Teste 6: Submissão protegida');
    try {
      const protectedSubmit = this.getProtectedSubmitExample();
  ...
  static getExpectedErrorStructures() {
    return {
      ...
      form_not_found: {
        ...
        message: "The form 'formulario_inexistente' does not exist or is inactive."
      },

      // 404 - Submissão não encontrada
      submit_not_found: {
        error: "submit_not_found",
        message: "The submit 'resposta_inexistente' was not found for the form 'formulario_abc'."
      },

      // 410 - Já removida anteriormente
      submit_already_removed: {
        error: "submit_already_removed",
        message: "The submit is already inactive. No further action has been taken."
      },

      // 403 - Formulário inativo
      inactive_form: {
        error: "inactive_form",
        message: "This form has been disabled and does not allow changes to your answers."
      },

      // 423 - Resposta protegida
      submit_blocked: {
        erro: "submit_blocked",
        mensagem: "The response is protected and cannot be removed due to retention policies."
      }
    };
  }
