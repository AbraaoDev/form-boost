...
export class FormSubmissionService {
  static async submitForm(
    ...
    try {
      ...
      if (submission.schemaVersion) {
        const requestedVersion = form.versions.find(
          (v: any) => Number(v.schema_version) === submission.schemaVersion,
        );
      ...
      const fields = Array.isArray(targetVersion.fields)
      ...
      const visibleFields = FormSubmissionService.getVisibleFields(
        typedFields,
        submission.answers,
      );
      ...
      const validationResult = FormValidationService.validateSubmission(
        typedFields,
        submission.answers,
      );
      ...
      const calculatedValues = FormSubmissionService.calculateFields(
        typedFields,
        submission.answers,
      );

      const prismaFormSubmissionsRepository =
  ...
  private static calculateFields(
    fields: Field[],
    ...
    // (topological sort)
    const sortedFields =
      FormSubmissionService.sortByDependencies(calculatedFields);

    for (const field of sortedFields) {
      if (field.type === 'calculated') {
        try {
          const value = FormSubmissionService.evaluateFormula(
            field.formula,
            {
              ...data,
              ...calculatedValues,
  ...
  static isFieldVisible(field: Field, data: Record<string, any>): boolean {
    ...
    try {
      const result = FormSubmissionService.evaluateFormula(
        field.conditional,
        data,
      );
  ...
  static getVisibleFields(fields: Field[], data: Record<string, any>): Field[] {
    return fields.filter((field) =>
      FormSubmissionService.isFieldVisible(field, data),
    );
  }
}
